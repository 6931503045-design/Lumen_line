...
- **`analyze.html` วิเคราะห์:** แท็บ ภาพรวม / รายหมวด
  - ภาพรวม: สรุปสถานะ, แนวโน้ม 6 เดือน (แท่งคู่ + แถวรายรับ/รายจ่ายกดดูรายละเอียด), ไทล์ "ใช้อย่างปลอดภัยได้" (อัตราใช้จ่าย, การคาดการณ์, กองทุนฉุกเฉิน — กดดูคำอธิบาย) + ป้ายความมั่นใจ (กดดูความหมาย), **เพดานการใช้จ่าย** (เปิด/ปิดแต่ละระดับได้ตรงนี้เลย + เลือกรูปแบบประหยัด/ปานกลาง), โดนัทสัดส่วนหมวดพร้อม popover, แผนเก็บเงิน (กดการ์ด = รายละเอียด, ⋮ = ดูรายละเอียด/ประวัติการเงิน/โอนเข้า/แก้ไข/ลบ, สร้างแผน 3 ขั้นตอน), จำลองผลกระทบก่อนซื้อ (+ หน้าอธิบายวิธีคิด)
  - รายหมวด: เลื่อนเดือน, ชิปเลือกหมวด, กราฟแท่งย้อนหลัง, ไทล์สถิติ 4 อัน (กดดูรายละเอียด), รายการแยกหมวด
- **`settings.html` ตั้งค่า:** โปรไฟล์จาก `/api/me`, **เพดานการใช้จ่าย** (รายวัน/สัปดาห์/เดือน เปิดปิดแยกกัน เลือกโหมดระบบคำนวณ/ตั้งเอง — ดูหัวข้อ 7.6), สวิตช์ ผู้ช่วย AI (สะท้อน `aiEnabled` จาก API แบบอ่านอย่างเดียว) / สรุปรายวัน (ล็อกไว้ ยังไม่มี API), อีเมลสำหรับ forward + ปุ่มสร้าง token ใหม่ (เรียก API จริง) + จำนวนอีเมลที่อ่านยอดไม่ออก
...
**2026-09-22 — เพิ่มการ์ดเพดานในหน้าวิเคราะห์ + รูปแบบ "ประหยัด/ปานกลาง"**

- การ์ด `#analyzeLimitsCard` (ระหว่างไทล์ "ใช้อย่างปลอดภัยได้" กับโดนัท) แสดง/แก้เพดานทั้ง 3 ระดับได้ในหน้านี้เลย ไม่ต้องไปหน้าตั้งค่า (ใช้ปุ่ม/แถวเดียวกับหน้าตั้งค่า คือคลาส `.limit-row*` และ `data-limit-toggle/-mode/-amount` เดิม แค่ผูก delegation เพิ่มใน `bindAnalyzePage()`)
- **รูปแบบของโหมด "ให้ระบบคำนวณ"** เลือกได้ 2 แบบ ผ่าน `SPENDING_PROFILE_META` — ตั้งอยู่ตรงหัวการ์ดนี้ (เก็บใน `jodtang.spendingLimitProfile`, มีผลกับทุกหน้า/ทุกระดับที่เป็นโหมด system):
...
- ทุกจุดที่ต้องรีเฟรชหลังแก้ค่าเพดาน (เปิด/ปิด, เปลี่ยนโหมด, เปลี่ยนจำนวนเงิน, เปลี่ยนรูปแบบ) รวมไว้ที่ `refreshSpendingLimitViews()` — เรียกฟังก์ชัน render ของทั้ง 3 จุด (การ์ดตั้งค่า, การ์ดวิเคราะห์, hero หน้าสรุป) โดยแต่ละอันเช็ค element เองว่าไม่มีก็ไม่ทำอะไร ปลอดภัยเรียกจากหน้าไหนก็ได้
- เดสก์ท็อป: เพิ่มแถว `'limits limits'` เต็มความกว้างใน `.analyze-grid` (ระหว่าง tiles กับ donut/plans)
