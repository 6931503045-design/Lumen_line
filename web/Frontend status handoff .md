...
- **`settings.html` ตั้งค่า:** โปรไฟล์จาก `/api/me`, สวิตช์ ผู้ช่วย AI (สะท้อน `aiEnabled` จาก API แบบอ่านอย่างเดียว) / สรุปรายวัน (ล็อกไว้ ยังไม่มี API), อีเมลสำหรับ forward + ปุ่มสร้าง token ใหม่ (เรียก API จริง) + จำนวนอีเมลที่อ่านยอดไม่ออก
