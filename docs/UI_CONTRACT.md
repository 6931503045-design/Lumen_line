...
| POST | `/api/plans/:planId/transfer` | โอนเงินเข้าแผน |
...
### `POST /api/plans/:planId/transfer`
```json
{ "amountSatang": 300000 }
```
โอนได้เฉพาะแผนที่ `status: "active"` — draft ต้องยืนยันก่อน (409) / ปิดไปแล้วโอนไม่ได้ (409)

คืน **201**:
```json
{
  "transactionId": "…",
  "progress": { …PlanProgress เหมือนใน GET /api/plans… },
  "justCompleted": false
}
```

`justCompleted: true` = รายการนี้ทำให้ครบเป้าพอดี แผนถูกปิดเป็น `completed` ให้แล้ว
และผู้ใช้ได้รับข้อความยินดีทาง LINE → **UI ควรฉลองด้วย**

⚖️ G7: บันทึกเป็น `type: "transfer"` ไม่ถูกนับเป็นรายจ่าย จึงไม่ไปหักยอด
"ใช้ได้ต่อวัน" ซ้ำ (S5.2 หัก `monthly_save` ออกไปแล้วรอบหนึ่ง)
รายการนี้จะโผล่ใน `/api/transactions` ด้วย `type: "transfer"` — **ถ้า UI ทำหน้าสรุปเอง
ต้องกรองออก** ไม่งั้นยอดรายจ่ายจะเกินจริง
...
> ✅ โอนเข้าแผนออม ทำเสร็จแล้ว (18 ก.ย. 2026) ดูหัวข้อ `POST /api/plans/:planId/transfer`
